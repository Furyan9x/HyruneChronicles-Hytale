...
public final class ItemRollCoordinator {
    ...
    public static void queueCraftedRoll(UUID playerUuid,
        ...
        if (HyruneConfigManager.getConfig().itemizationDebugLogging) {
            ...
            LOGGER.at(Level.INFO).log("[Itemization] Queue crafted roll player=" + playerUuid
                + ", item=" + normalized
                + ", added=" + quantity
                + ", pending=" + pendingQty);
        }
    ...
    public static void applyPendingCraftRolls(Player player) {
        if (player == null) {
            return;
        }
        ...
        if (debug) {
            LOGGER.at(Level.INFO).log("[Itemization] Applying pending rolls for player=" + playerUuid + ", entries=" + pending);
        }
        ...
        for (short slot = 0; slot < capacity; slot++) {
            ...
            ItemStack rolled = ItemGenerationService.rollIfEligible(
                ...
                ItemGenerationContext.crafting(
                    "craft_queue_apply",
                    playerUuid.toString(),
                    itemId,
            ...
            appliedStacks++;
            if (debug) {
                LOGGER.at(Level.INFO).log("[Itemization] Applied roll slot=" + slot
        ...
        if (debug) {
            LOGGER.at(Level.INFO).log("[Itemization] Apply complete player=" + playerUuid
