...
import dev.hytalemodding.origins.util.RepairConfig;
...
public class RepairBenchPage extends InteractiveCustomUIPage<RepairBenchPage.RepairBenchData> {
    ...
    private void handleRepair(Ref<EntityStore> ref, Store<EntityStore> store, String slotText) {
        ...
        // Use RepairConfig to determine repair requirements
        RepairConfig.RepairMaterial repairInfo = RepairConfig.getRepairInfo(
                stack.getItemId(),
                currentDurability,
                maxDurability
        );

        ItemStack payment = new ItemStack(repairInfo.getMaterialItemId(), repairInfo.getQuantity());
        var tx = container.removeItemStack(payment);
        if (tx == null || !tx.succeeded()) {
            String materialKey = payment.getItem().getTranslationKey();

            // Create the message with the key, then chain params
            Message errorMsg = Message.translation("server.repair.missing_material.name")
                    .param("quantity", repairInfo.getQuantity())
                    .param("material", Message.translation(materialKey)); // Nesting the item name
        ...
        ItemStack updated = stack.withIncreasedDurability(restoreAmount);
    ...
    private void populateRepairList(UICommandBuilder cmd, UIEventBuilder evt, Player player) {
        ...
        for (short slot = 0; slot < capacity; slot++) {
            ...
            // Use RepairConfig to get repair requirements
            RepairConfig.RepairMaterial repairInfo = RepairConfig.getRepairInfo(
                    stack.getItemId(),
                    currentDurability,
                    maxDurability
            ...
            entries.add(new RepairEntry(slot, stack, repairInfo));
        }
        ...
        for (RepairEntry entry : entries) {
            ...
            int playerHasCount = getInventoryCount(player, entry.repairInfo.getMaterialItemId());
            boolean canAfford = playerHasCount >= entry.repairInfo.getQuantity();
            ...
            cmd.set(rowRoot + " #CostIcon.ItemId", entry.repairInfo.getMaterialItemId());
            cmd.set(rowRoot + " #ItemCost.Text", "x" + entry.repairInfo.getQuantity());
    ...
    private static final class RepairEntry {
        ...
        private final RepairConfig.RepairMaterial repairInfo;

        private RepairEntry(short slot, ItemStack stack, RepairConfig.RepairMaterial repairInfo) {
            this.slot = slot;
            this.stack = stack;
